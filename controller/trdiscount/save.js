...
const trdiscount = require('../../model/trdiscount');


const save = async (req, res, next) => {
    try {
        var data = req.body;
        var where = {discountId: (data['discountId']) ? data['discountId'] : 0};
        await dbManager.createOrUpdate(trdiscount, data, where).then(result => {
