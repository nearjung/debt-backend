...
const mscustomerinfo = require('../../model/mscustomerinfo');


const save = async (req, res, next) => {
    try {
        var data = req.body;
        var where = {customerInfoId: (data['customerInfoId']) ? data['customerInfoId'] : 0};
        await dbManager.createOrUpdate(mscustomerinfo, data, where).then(result => {
