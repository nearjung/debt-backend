...
const sytransactionstatus = require('../../model/sytransactionstatus');


const save = async (req, res, next) => {
    try {
        var data = req.body;
        var where = {transactionStatusId: (data['transactionStatusId']) ? data['transactionStatusId'] : 0};
        await dbManager.createOrUpdate(sytransactionstatus, data, where).then(result => {
