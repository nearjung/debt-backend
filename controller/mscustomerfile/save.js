...
const mscustomerfile = require('../../model/mscustomerfile');


const save = async (req, res, next) => {
    try {
        var data = req.body;
        var where = {customerFileId: (data['customerFileId']) ? data['customerFileId'] : 0};
        await dbManager.createOrUpdate(mscustomerfile, data, where).then(result => {
