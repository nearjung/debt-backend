...
const syfollowtype = require('../../model/syfollowtype');


const save = async (req, res, next) => {
    try {
        var data = req.body;
        var where = {followTypeId: (data['followTypeId']) ? data['followTypeId'] : 0};
        await dbManager.createOrUpdate(syfollowtype, data, where).then(result => {
