...
const syfollowstatus = require('../../model/syfollowstatus');


const save = async (req, res, next) => {
    try {
        var data = req.body;
        var where = {followStatus: (data['followStatus']) ? data['followStatus'] : 0};
        await dbManager.createOrUpdate(syfollowstatus, data, where).then(result => {
